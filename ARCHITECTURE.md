...
│  Components (30+)   └────────────────┘    └──────────────┘  │   │
│  ┌──────────────┐   ┌────────────────┐                      │   │
│  │ ChatPanel    │──→│ useChat (375)  │──── WebSocket ───────┼─┐ │
...
|--------|-------|-----------------|
| runtime.py | ~365 | CLI subprocess lifecycle, budget tracking, circuit breaker |
| eval_service.py | ~312 | EvalCase/EvalRun CRUD, batch execution, comparison |
| graph_service.py | ~303 | LangGraph StateGraph: 3 nodes, 2 routing functions, checkpoint |
...
| Hooks | 9 | useChat (375 строк), useEvaluations, useAgents и др. |
| Components | 30+ | ChatPanel, AgentForm, EvalRunDetail и др. |
| API Layer | 7 | client.ts + resource modules |
...
7. graph.astream() → run_agent_node:
     a. runtime.send_message():
        → kill ALL existing processes (safety reset)

...
from app.services.orchestrator_service import format_handoff_instructions, handle_handoff
...
async def _handle_messages(
    ...
    while True:
        ...
        if msg_type == "message":
            ...
            await _stream_response(websocket, db, session_id, content, agent, handoff_targets)
            continue
...
async def _stream_response(
    ...
    # Передать управление оркестратору — он разберётся с handoff-блоком
    if full_text:

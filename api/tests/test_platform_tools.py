...
import mcp_server.tools.platform as platform_module
...
class TestApiHelper:
    ...
    async def test_api_get_returns_json(self):
        ...
        with patch("mcp_server.tools.platform.httpx.AsyncClient") as mock_client_cls:
            mock_client = AsyncMock()
            mock_client.get = AsyncMock(return_value=response)
            mock_client_cls.return_value.__aenter__ = AsyncMock(return_value=mock_client)
            mock_client_cls.return_value.__aexit__ = AsyncMock(return_value=False)

            from mcp_server.tools.platform import _api
...
class TestPlatformToolsFunctions:
    ...
    async def test_list_businesses(self, registered_platform_tools):
        ...
        with patch("mcp_server.tools.platform._api", new=AsyncMock(return_value=businesses)):
    ...
    async def test_create_business(self, registered_platform_tools):
        ...
        with patch("mcp_server.tools.platform._api", new=AsyncMock(return_value=business)):
    ...
    async def test_create_business_with_description(self, registered_platform_tools):
        ...
        with patch("mcp_server.tools.platform._api", new=api_mock):
    ...
    async def test_list_products(self, registered_platform_tools):
        products = [{"id": "p1"}]
        with patch("mcp_server.tools.platform._api", new=AsyncMock(return_value=products)):
    ...
    async def test_create_product(self, registered_platform_tools):
        ...
        with patch("mcp_server.tools.platform._api", new=AsyncMock(return_value=product)):
    ...
    async def test_list_teams(self, registered_platform_tools):
        ...
        with patch("mcp_server.tools.platform._api", new=AsyncMock(return_value=teams)):
    ...
    async def test_create_team(self, registered_platform_tools):
        team = {"id": "t1", "name": "Dev"}
        with patch("mcp_server.tools.platform._api", new=AsyncMock(return_value=team)):
    ...
    async def test_create_agent(self, registered_platform_tools):
        ...
        with patch("mcp_server.tools.platform._api", new=AsyncMock(return_value=agent)):

    ...
    return (
      <div className="flex items-center gap-1.5">
        ...
        <button type="button" onClick={() => { setEditName(selected?.name ?? ""); setEditing(true); }}
          className="text-[11px] text-gray-400 hover:text-blue-600 px-1">✏️</button>
      </div>
    );
  }
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 px-1">
        ...
        {workflow.isolation_mode === "worktree" && (
          <label className="flex items-center gap-1.5 cursor-pointer">
            <input
              type="checkbox"
              checked={workflow.auto_merge ?? false}
              onChange={(e) => onUpdateWorkflow(workflow.id, { auto_merge: e.target.checked })}
              className="rounded border-gray-300"
            />
            <span className="text-[11px] text-gray-600">Авто-merge</span>
          </label>
        )}
      </div>
      <div>
        ...
        <div>
          {steps.map((step, idx) => {
            ...
            const showLine = !isLast || agentReturns.length > 0;
            return (
              <div key={step.agent.id}>
                ...
                <div className="flex gap-3">
                  <div className="flex flex-col items-center w-3 shrink-0">
                    ...
                    {showLine && <div className="w-px flex-1 bg-gray-300" />}
                  </div>
                  <div className="flex-1 -mt-0.5 pb-2">
                    ...
                    {isLast && agentReturns.length === 0 && (
                      <p className="text-[10px] text-gray-400 mt-1 italic">конец цепочки</p>
                    )}
                  </div>
                </div>
              </div>
